    ...
    for epoch in range(num_epochs):
        ...
        for batch_inputs, batch_labels in generator_tqdm:
            with tf.GradientTape() as tape:
                ...
                #assign lambda value as in paper
                regularization_lambda = 1/100000
                regularization = 0
                #iterate for each of trainable variables to compute L2 norm
                for var in model.trainable_variables:

                    regularization += tf.nn.l2_loss(var)

                #multiply with lambda to compute final value
